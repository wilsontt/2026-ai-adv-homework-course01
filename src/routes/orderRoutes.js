...
const { buildAioCheckOutParams, buildMerchantTradeNo, queryTradeInfo } = require('../services/ecpayService');
...
router.post('/:id/payment', (req, res) => {
  ...
  // 遞增 payment_no 產生唯一 MerchantTradeNo
  const newPaymentNo = (order.payment_no || 0) + 1;
  db.prepare('UPDATE orders SET payment_no = ? WHERE id = ?').run(newPaymentNo, order.id);

  const baseUrl = process.env.BASE_URL || 'http://localhost:3001';
  const { actionUrl, params } = buildAioCheckOutParams(order, items, newPaymentNo, baseUrl);

  res.json({
    data: { action_url: actionUrl, params },
    error: null,
    message: '付款表單產生成功'
  });
});
...
/**
 * @openapi
 * /api/orders/{id}/payment/query:
 *   post:
 *     summary: 查詢綠界 ECPay 付款結果
 *     tags: [Orders]
 *     security:
 *       - bearerAuth: []
 *     parameters:
 ...
 *         description: 訂單不存在
 */
router.post('/:id/payment/query', async (req, res) => {
  const userId = req.user.userId;
  ...
  if (order.status !== 'pending') {
    ...
    return res.json({
      data: { ...order, items },
      error: null,
      message: order.status === 'paid' ? '訂單已付款' : '訂單付款失敗'
    });
  }

  const paymentNo = order.payment_no || 0;
  if (paymentNo === 0) {
    return res.status(400).json({
      data: null,
      error: 'INVALID_STATUS',
