...
7. tests/ecpay.test.js           # CheckMacValue 官方向量、buildMerchantTradeNo、付款 API 整合、notify 回調整合
```
...
| `ecpay.test.js` | 需 seed 商品（找 `stock > 0`）+ 能 `registerUser`；notify 整合測試以官方測試向量 HashKey/HashIV 自產 CheckMacValue，模擬綠界回調，不需真實刷卡 |
...
8. **ECPay notify 測試不打真實綠界**：`ecpay.test.js` 透過 `POST /api/ecpay/notify` 模擬綠界 Server Notify，以官方測試向量 HashKey/HashIV 自行產生合法 CheckMacValue。真正呼叫綠界 `QueryTradeInfo/V5` 的失敗分支（500 `ECPAY_ERROR`）未覆蓋，若要測試請自行 mock `global.fetch` 或在 `src/services/ecpayService.js` 注入 client。
