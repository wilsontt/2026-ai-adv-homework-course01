# backend-project — 花卉電商 Demo

一個以 Express 4 為核心的電商示範系統，提供會員註冊登入、商品瀏覽、購物車（訪客/會員雙模式）、訂單建立、綠界 ECPay 金流付款（含模擬付款開發模式），以及後台商品/訂單管理。API 均以統一回應信封格式輸出，並透過 swagger-jsdoc 的 `@openapi` 註解產生 OpenAPI 3.0.3 規格。
